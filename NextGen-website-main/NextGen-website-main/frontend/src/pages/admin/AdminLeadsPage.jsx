  ...
  return (
    <div className="p-8 lg:p-12" data-testid="admin-leads-page">
      <div className="flex items-baseline justify-between mb-10">
        ...
        <div className="text-sm text-[#1B1D22]/70">{leads.length} total</div>
      </div>
        ...
        {["all", ...STATUS_OPTIONS].map((s) => (
          <button
            ...
            className={`text-[10px] tracking-[0.22em] uppercase px-4 py-2.5 transition-colors ${
              filter === s ? "bg-[#0B0B0D] text-[#F7F5F2]" : "border border-[#0B0B0D]/20 hover:border-[#0B0B0D]"
            }`}
          >
      ...
      <div className="border-t border-b border-[#0B0B0D]/10 divide-y divide-[#0B0B0D]/10">
        {loading ? (
          ...
          <p className="py-12 text-center text-[#1B1D22]/60">No leads to show.</p>
        ) : (
          visible.map((lead) => (
            <div key={lead.id} className="py-6 grid grid-cols-12 gap-4 items-start" data-testid={`lead-row-${lead.id}`}>
              <div className="col-span-3">
                ...
                <div className="text-xs text-[#1B1D22]/60 mt-1">{new Date(lead.created_at).toLocaleString("en-IN")}</div>
                <div className="text-[10px] tracking-[0.22em] uppercase text-[#6D4C41] mt-2">via {lead.source}</div>
              </div>
              ...
              <div className="col-span-3 text-sm">
                {lead.project_type && <div><span className="text-[#6D4C41]">Project:</span> {lead.project_type}</div>}
                {lead.budget && <div><span className="text-[#6D4C41]">Budget:</span> {lead.budget}</div>}
                {lead.timeline && <div><span className="text-[#6D4C41]">Timeline:</span> {lead.timeline}</div>}
                {lead.location && <div><span className="text-[#6D4C41]">Location:</span> {lead.location}</div>}
                {lead.message && <p className="mt-2 text-[#1B1D22]/75 italic">"{lead.message}"</p>}
              </div>
              <div className="col-span-2">
                <select
                  ...
                  className="bg-transparent border border-[#0B0B0D]/20 px-3 py-2 text-[10px] tracking-[0.22em] uppercase w-full"
                >
                  {STATUS_OPTIONS.map((s) => <option key={s} value={s}>{s}</option>)}
                </select>
              </div>
              <div className="col-span-1 text-right">
                <button onClick={() => remove(lead.id)} data-testid={`lead-delete-${lead.id}`} className="text-[#1B1D22]/40 hover:text-red-700 transition-colors">
                  <Trash2 size={16} />
                </button>
              </div>
            </div>
