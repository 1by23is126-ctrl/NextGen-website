...
export default function AdminLoginPage() {
  ...
  return (
    <div data-testid="admin-login-page" className="min-h-screen flex bg-[#F7F5F2]">
      <div className="hidden md:flex md:w-1/2 bg-[#0B0B0D] text-[#F7F5F2] relative overflow-hidden">
        <img
        ...
        <div className="relative z-10 p-12 lg:p-16 flex flex-col justify-between w-full">
          <Link to="/" className="flex items-center gap-3">
            ...
            <span className="font-serif text-xl">NextGen <span className="italic text-[#C9A86A]">Interiors</span></span>
          </Link>
          <div>
            <div className="ngi-overline text-[#C9A86A] mb-4"><span className="ngi-rule" />Studio Control</div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        ...
        >
          ...
          <div className="space-y-6">
            ...
            <div>
              <label className="text-[10px] tracking-[0.22em] uppercase text-[#6D4C41]">Password</label>
              <input
                required
                type="password"
                value={password}
            ...
            <button
              ...
              className="w-full bg-[#0B0B0D] text-[#F7F5F2] hover:bg-[#C9A86A] hover:text-[#0B0B0D] px-8 py-4 text-[11px] tracking-[0.22em] uppercase transition-colors disabled:opacity-60"
            >
              {loading ? "Signing in…" : "Sign in"}
            </button>
          </div>

          <Link to="/" className="mt-12 inline-block text-[10px] tracking-[0.22em] uppercase text-[#1B1D22]/60 ngi-link-underline">← Back to site</Link>
        </motion.form>
      </div>
    </div>
  );
